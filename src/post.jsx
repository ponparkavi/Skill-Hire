...
function Post({ posts, setPosts }) {
  ...
  const addImageBlock = (file) => {
    const reader = new FileReader();
    reader.onload = e => setBlocks([...blocks, { type: "image", src: e.target.result }]);
    reader.readAsDataURL(file);
  };
  const addVideoBlock = (file) => {
    const reader = new FileReader();
    reader.onload = e => setBlocks([...blocks, { type: "video", src: e.target.result }]);
    reader.readAsDataURL(file);
  };
  ...
  return (
    <Box className="post-page">
      <form onSubmit={handleSubmit} className="post-content">
        <Typography variant="h5" mb={2}>Create a Post</Typography>
        {blocks.map((b,i)=>{
          if(b.type==="text")
            return <TextField key={i} fullWidth multiline minRows={2} placeholder="Write something..." value={b.content} onChange={e=>updateTextBlock(i,e.target.value)} sx={{mb:2}}/>
          if(b.type==="image") return <img key={i} src={b.src} className="image-preview" alt={`img-${i}`}/>
          if(b.type==="video") return <video key={i} src={b.src} controls className="video-preview"/>
          return null;
        })}

        <Box sx={{display:"flex",gap:2,mb:3}}>
          <Button variant="outlined" onClick={addTextBlock}>Add Text</Button>
          <Button variant="outlined" component="label">Add Image<input type="file" hidden accept="image/*" onChange={e=>addImageBlock(e.target.files[0])}/></Button>
          <Button variant="outlined" component="label">Add Video<input type="file" hidden accept="video/*" onChange={e=>addVideoBlock(e.target.files[0])}/></Button>
        </Box>
