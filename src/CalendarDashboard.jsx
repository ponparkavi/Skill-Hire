...
export default function CalendarDashboard() {
  ...
  const [sessions, setSessions] = useState([
    {
      id: "1",
      title: "React Basics - John",
      start: "2025-10-08T17:00:00",
      status: "accepted",
    },
    {
      id: "2",
      title: "Python Coaching - Mary",
      start: "2025-10-10T10:00:00",
      status: "declined",
    },
    {
      id: "3",
      title: "UI Design - Ethan",
  ...
  const ongoingCount = sessions.filter(
    (s) => s.status === "ongoing" || s.status === "accepted"
  ).length;

  const handleAddSession = (e) => {
    e.preventDefault();
    if (!newSession.learner || !newSession.subject || !newSession.datetime)
      return;
      ...
      <div className="sidebarz sdb">
        ...
        {displayedSessions.length === 0 ? (
          ...
          displayedSessions.map((s) => (
            ...
            >
              <div>
                <strong>{s.title.split(" - ")[0]}</strong> with{" "}
                {s.title.split(" - ")[1]}
                <br />
                <small>{formatDateTime(s.start)}</small>
                <span
                  className={`tag ${s.status}`}
                  style={{ marginLeft: "5px" }}
                >
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleAddSession}>
              <div>
                ...
                <input
                  type="text"
                  value={newSession.learner}
                  onChange={(e) =>
                    setNewSession({ ...newSession, learner: e.target.value })
                  }
                  required
                />
              </div>
